...
WHERE T.MEMBER = I.ID and T.TBSP_ID = TSDETAIL.TBSPACEID WITH UR`
	...
	lockWaitDetailsQuery = `SELECT
	l.AGENT_ID,
	l.APPLICATION_HANDLE,
	COALESCE(a.APPLICATION_NAME, '') AS APPLICATION_NAME,
	COALESCE(a.SESSION_AUTH_ID, '') AS AUTH_ID,
	l.LOCK_MODE,
	l.LOCK_MODE_REQUESTED,
	l.LOCK_OBJECT_TYPE,
	l.LOCK_WAIT_TIME,
	COALESCE(l.TABSCHEMA, '') AS TABSCHEMA,
	COALESCE(l.TABNAME, '') AS TABNAME,
	COALESCE(SUBSTR(a.STMT_TEXT, 1, 500), '') AS STMT_TEXT,
	l.LOCK_HOLDER_APPLICATION_HANDLE
	FROM TABLE(MON_GET_LOCKS(NULL, -2)) l
	LEFT JOIN TABLE(MON_GET_ACTIVITY(NULL, -2)) a
		ON l.APPLICATION_HANDLE = a.APPLICATION_HANDLE
	WHERE l.LOCK_STATUS = 'W'
		AND l.LOCK_WAIT_TIME >= ?
